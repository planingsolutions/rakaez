    ...
    <table id="files_list" class="table table-striped dt-responsive  " style="text-align: right !important" >
        ...
        <thead class="thead_dark">
                    <tr>
                        ...
                        <th class="th_text">  البراجراف الاول</th>
                        <th class="th_text">البراجراف الثاني</th>
        ...
        <tbody>
                    @foreach ($post as $val)
                        <tr>
                            ...
                            <td class="pargraph2">{{$val->pargraph1}}</td>
                            <td class="pargraph2">{{$val->pargraph2}}</td>
                            <td class="update">{{$val->updated_at}}</td>
                            <td><a href="{{route('admainpanal.serviceEdit',$val->id)}}" class="btn btn-outline-success" style="width: -moz-available;">تعديل</a> </td>                    
                        </tr>      
                    @endforeach
        </tbody>
